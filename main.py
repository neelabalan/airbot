...
def get_messages_for_posting( feed_data, preference, filter_function ):
    ...
    for entry in entries:
        ...
        if filtrate in preference and published.split(',')[0] == date.today().strftime( '%b %d'):
            messages.append( 
                construct_message( 
                    entry     = entry,
                    published = published,
...
def run( config_name = 'config.toml' ):
    ...
    logger.info( 'local_preference - {}, national_preference'.format( local_preference, national_preference ))


    local_feed_data    = get_feed_data( urls[ 'local' ] ) if local_preference else list() 
    national_feed_data = get_feed_data( urls[ 'national' ]) if national_preference else list() 
